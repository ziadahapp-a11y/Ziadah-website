...
async function fetchZidProducts(baseUrl: string): Promise<{ products: ScrapedProduct[]; currency: string; currencySymbol: string }> {
  ...
  while (true) {
    const apiUrl = `${cleanUrl}/api/v1/products?page=${page}&page_size=${PAGE_SIZE}`;
    const res = await fetch(apiUrl, {
      headers: { ...HEADERS, "Accept": "application/json" },
      signal: AbortSignal.timeout(FETCH_TIMEOUT),
    });

    if (!res.ok) throw new Error(`Zid API returned ${res.status} at ${apiUrl}`);
    ...
    // Capture currency from first product on first page
    if (page === 1 && items[0]) {
    ...
    logger.info({ page, fetched: items.length, total, baseUrl }, "Zid: fetched page");

    if (!hasMore || products.length >= total) break;
    page++;

    // Safety: cap at 500 products
