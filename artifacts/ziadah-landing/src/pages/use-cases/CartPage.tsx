...
function CartHeroContent() {
  ...
  return (
    <div dir={dir} style={{ fontFamily: "var(--font, system-ui)", padding: "0 2px" }}>
      ...
      {/* progress */}
      <div style={{ background: "rgba(139, 92, 246,.08)", border: "1px solid rgba(139, 92, 246,.25)", borderRadius: 10, padding: "8px 10px", marginBottom: 10 }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: "#8b5cf6", marginBottom: 5 }}>{tr.freeShipHint}</div>
        <div style={{ height: 5, background: "rgba(139, 92, 246,.15)", borderRadius: 99 }}>
          <div style={{ height: "100%", width: "82%", background: "linear-gradient(90deg, #8b5cf6, #8b5cf6)", borderRadius: 99 }} />
        </div>
      ...
      {tr.lineItems.map((item, i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 0", borderBottom: "1px solid var(--b1)" }}>
          ...
          <div style={{ fontSize: 11, fontWeight: 900, color: "#8b5cf6" }}>{item.price}</div>
        </div>
      ))}
      {/* suggestion */}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10, padding: "8px 0", borderTop: "1px solid var(--b1)" }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 900, color: "#8b5cf6" }}>{tr.grandTotal}</span>
      </div>
      <button type="button" style={{ width: "100%", marginTop: 8, background: "linear-gradient(135deg, #8b5cf6, #7c3aed)", color: "#fff", border: "none", borderRadius: 10, padding: "9px 0", fontSize: 12, fontWeight: 900, cursor: "pointer" }}>
        {tr.checkoutCta}
      </button>
    </div>
  );
...
const data: UseCasePageData = {
  ...
  stats: [
    ...
    { value: "+22%", label: "متوسط قيمة الطلب", color: "#8b5cf6" },
    { value: "+31%", label: "الطلبات تتجاوز عتبة الشحن", color: "#8b5cf6" },
    { value: "+18%", label: "معدل إتمام الشراء", color: "#06b6d4" },
  ],
  exampleScenario: {
    title: "عميل في سلة بقيمة 170 ر.س",
    steps: [
      "العميل أضاف منتجي تجميل بقيمة 170 ر.س.",
      "يكتشف زيادة أن عتبة الشحن المجاني عند 200 ر.س — الفجوة 30 ر.س فقط.",
      "يظهر مباشرة: 'أضف 30 ر.س وتحصل على شحن مجاني' + بلسم شعر بـ 32 ر.س كاقتراح.",
  ...
  statsEn: [
    ...
    { value: "+22%", label: "Average order value", color: "#8b5cf6" },
