...
const data: UseCasePageData = {
  ...
  stats: [
    { value: "+٤١٪", label: "متوسط قيمة الطلب", color: "#a855f7" },
    { value:"+٢٨٪", label: "معدل قبول اقتراح الترقية", color: "#06b6d4" },
    { value: "+٣٥٪", label: "رضا العملاء على المدى البعيد", color: "#10b981" },
    { value: "+٢٢٪", label: "معدل تكرار الشراء", color: "#f59e0b" },
  ],
  exampleScenario: {
    title: "أمثلة تطبيقية حقيقية عبر قطاعات",
    steps: [
      "🎧 إلكترونيات: عميل ينظر لسماعة بـ ١٢٠ ريال → يرى عرض 'سماعة بـ ١٩٠ ريال — عزل صوت فعّال + ٢٠ ساعة بطارية + ضمان سنتان' مع مقارنة مباشرة.",
      "💐 عطور: عميلة تنظر لعطر ٣٠ مل → تجد 'النسخة ١٠٠ مل بـ ٣١٠ ريال (توفر ٣٥٪ في سعر الـ مل)'.",
      "🌿 عناية: عميل اختار كريم أساسي → يظهر كريم بريميوم بنفس الوزن وبـ ٣٠ ريال إضافية مع قائمة مكونات نشطة أكثر.",
      "📚 كورسات: مشترك في باقة أساسية → يرى ترقية للباقة المتقدمة بفارق ٨٠ ريال مع ٣ كورسات إضافية وشهادة.",
    ],
    result: "معدل قبول عرض الترقية يتراوح بين ٢٢ و٣٥٪ عند تقديمه بالطريقة الصحيحة مع إبراز القيمة — وكل قبول يرفع الإيراد مباشرة.",
  },
  extraSections: (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
            <div style={{ padding: "24px 28px", background: "rgba(16,185,129,.05)", border: "1px solid rgba(16,185,129,.15)", borderRadius: 14 }}>
              ...
              <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: 8 }}>
                {["الفارق في السعر ٢٠-٥٠٪ وليس أكثر", "الفوائد الإضافية واضحة وملموسة", "العميل له تاريخ شراء في الفئة المتميزة", "يُعرض قبل إضافة المنتج للسلة"].map((item, i) => (
            ...
            <div style={{ padding: "24px 28px", background: "rgba(225,29,72,.05)", border: "1px solid rgba(225,29,72,.15)", borderRadius: 14 }}>
              ...
              <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: 8 }}>
                {["الفارق أكبر من ٥٠٪ بدون مبرر واضح", "النسخة المُقترحة في فئة مختلفة كلياً", "العميل في مرحلة متقدمة من عملية الدفع", "يُعرض بشكل مكثف أكثر من مرة"].map((item, i) => (
                  <li key={i} style={{ display: "flex", gap: 8, fontSize: 14, color: "var(--tm)", alignItems: "flex-start" }}>
