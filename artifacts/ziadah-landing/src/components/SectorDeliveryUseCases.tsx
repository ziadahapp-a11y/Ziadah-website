...
function VisRow({ row, isAr }: { row: VisRowDef; isAr: boolean }) {
  ...
  const icon = row.icon ?? "📦";
  const borderO = row.className === "highlight-o" ? "rgba(34, 197, 125,.35)" : row.className === "highlight-g" ? "rgba(0,212,160,.3)" : "var(--b2)";
  return (
    <div
      ...
      {tag ? (
        <span
          ...
          style={{
            ...
            whiteSpace: "nowrap",
            background: row.tagClass === "tag-gold" ? "rgba(245,192,48,.12)" : row.tagClass === "tag-g" ? "rgba(0,212,160,.1)" : "rgba(34, 197, 125,.08)",
