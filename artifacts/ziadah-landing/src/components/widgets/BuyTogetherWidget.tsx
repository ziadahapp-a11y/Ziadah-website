  ...
  return (
    <UseCaseWidgetPreview title="منتجات يتم شراؤها معاً" subtitle="اشتروا معاً">
      <div style={{ marginBottom: 10 }}>
        <div style={{ fontSize: 14, color: "rgba(255,255,255,.45)", marginBottom: 10 }}>منتجات يفضلها العملاء معاً</div>
          ...
          {products.map((p, i) => (
            <div key={i} onClick={() => toggle(i)} style={{
              ...
              <div style={{
                ...
                {p.checked && <span style={{ color: "#fff", fontSize: 14, fontWeight: 900 }}>✓</span>}
              </div>
              <div style={{
                width: 36,
                height: 36,
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: "flex", gap: 5, alignItems: "center", marginTop: 2 }}>
                  <span style={{ fontSize: 14, fontWeight: 800, color: "#fff" }}>﷼{p.price}</span>
                  {p.originalPrice && <span style={{ fontSize: 14, color: "rgba(255,255,255,.35)", textDecoration: "line-through" }}>﷼{p.originalPrice}</span>}
                </div>
              </div>
              {p.tag && <div style={{ fontSize: 14, padding: "2px 7px", borderRadius: 20, background: "rgba(124,58,237,.2)", color: "#c084fc", fontWeight: 700, flexShrink: 0 }}>{p.tag}</div>}
            </div>
          ))}
        </div>
      </div>
